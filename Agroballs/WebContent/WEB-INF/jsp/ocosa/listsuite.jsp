...
<input type="submit" value="回首頁">        
   ...
   <h1>專案名稱：<font color="brown">${planname}</font></h1>
   <br>
    <table border="1">
        <tr>
            <th>行動措施</th>
        ...
        <c:forEach var="suite" items="${suiteList }" >
            <tr>
               ...
               <td style="width:5%">${suite.actioner}</td>                
                ...
                <td>
         		<c:url var="editsuiteUrl" value="/ocosa/editsuite/${suite.id}" />
                <form:form  action="${editsuiteUrl}" method="get">
					<input type="submit" value="编輯左列行動措施">
					</form:form></td>
                <td><c:url var="delsuiteUrl" value="/ocosa/delsuite/${suite.id}" />
                <form:form  action="${delsuiteUrl}" method="get">
					<input type="submit" value="刪除本項行動措施">   
					</form:form></td> 
